...
def download_youtube_persian_asr(
    output_root: Path,
    *,
    revision: str = "main",
    workers: int = 8,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Download the PerSets YouTube Persian ASR metadata and tar shards from Hugging Face."
    )
    parser.add_argument(
        "--output-root",
        type=Path,
    ...
    audit = download_youtube_persian_asr(
        args.output_root,
        revision=args.revision,
        workers=args.workers,
        force=args.force,
    )
    print_download_audit(YOUTUBE_DATASET, audit)
    return 0
