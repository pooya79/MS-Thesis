"""Fine-tune ``openai/whisper-medium`` for Persian ASR.
...
Whisper Medium defaults, including gradient checkpointing for lower VRAM use.
"""
...
DEFAULT_CONFIG_MEDIUM: dict[str, Any] = deep_merge(
    deepcopy(DEFAULT_CONFIG),
    {
        "model": {"name": "openai/whisper-medium"},
        "run": {"output_dir": "models/asr/whisper-medium/runs"},
        "training": {
...
def load_training_config(config_path: Path) -> dict[str, Any]:
    return _load_training_config(config_path, DEFAULT_CONFIG_MEDIUM)
...
def run_training(
    ...
    return _run_training(
        ...
        run_dir_override,
        resume_override,
        defaults=DEFAULT_CONFIG_MEDIUM,
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Fine-tune openai/whisper-medium from a YAML config. "
