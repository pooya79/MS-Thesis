...
number of sampled examples per epoch and the joint sampling fraction/balancing
settings. Resume refuses a changed budget or sampling distribution. Older budgets
without sampler metadata remain resumable with historical shuffled sampling;
weighted budgets without recorded distribution settings require a new run directory.
Explicit and default sampling settings are compared by their resolved values,
including the actual draw count. As before,
