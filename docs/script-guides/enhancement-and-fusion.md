...
Each stage records `config/<stage>_budget.json`, including the sampler type and
number of sampled examples per epoch and the joint clean-subset fraction. Resume refuses a changed budget or sampling distribution. Older budgets
without sampler metadata remain resumable with historical shuffled sampling;
Previously weighted runs require a new run directory for this changed recipe.
The clean fraction and actual selected row count are checked on resume. As before,
an epoch recipe cannot resume an old step-only `last.pt` without a recorded
